...
The Google Health API stores every nutrient in grams and records the unit the user
actually typed alongside it, so a 95mg caffeine entry can be displayed as "95mg"
rather than "0.095g".
"""
...
# "mcg" and "ug" are the ASCII spellings of µg that labels and keyboards fall back to.
_SPELLINGS = {
...
def resolve_unit(unit: Optional[str]) -> WeightUnit:
    ...
    if not unit or not unit.strip():
        raise UnknownUnitError("A unit is required: write 95mg or 0.095g, not a bare number.")

    resolved = _UNIT_BY_SPELLING.get(unit.strip().lower())
    if resolved is None:
        raise UnknownUnitError(f"Unrecognised unit {unit.strip()!r}: use g, mg or µg.")
    return resolved


def parse_weight(value: float, unit: Optional[str]) -> Tuple[float, WeightUnit]:
    """Convert a written quantity into grams, keeping the unit it was written in."""
...
def format_grams(grams: float, unit: Optional[WeightUnit]) -> str:
    """Render grams in the unit it was written in, or the most legible one if unknown."""
    display_unit = unit or _readable_unit(grams)
    scaled = grams / _GRAMS_PER_UNIT[display_unit]

    # Round before trimming: floating point leaves 450.00000000000006 for 0.45mg.
